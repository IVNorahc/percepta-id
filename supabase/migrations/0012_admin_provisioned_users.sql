-- Percepta ID — Comptes provisionnés par l'admin.
--
-- La création de comptes est désormais réservée à l'admin (plus d'inscription
-- publique). Quand l'admin crée un compte, il choisit l'entreprise ET le rôle :
-- le trigger ne doit donc PAS créer une nouvelle entreprise dans ce cas.
--
-- Distinction via user_metadata :
--   - 'company_id' présent  → compte provisionné : entreprise + rôle imposés
--   - sinon (legacy)        → auto-inscription : crée une nouvelle entreprise
...
begin
  ...
  -- ── (Legacy) auto-inscription : crée une nouvelle entreprise ────────────────
